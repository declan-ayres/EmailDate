...
public class EmailDate {
	...
	try {
			...
			final GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(JSON_FACTORY, new FileReader(
			...
			GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(httpTransport, JSON_FACTORY,
					...
					"2T56GjlkKVpKU5rKSJi0ZWSj", Collections.singleton(GmailScopes.GMAIL_MODIFY)).setDataStoreFactory(
			...
			ListMessagesResponse list = gmail.users().messages().list("bullhorn@314ecorp.com")
					.setQ("in:inbox from:" + "(+" + useremail + ") after:" + queryDateString).execute();
			...
			for (int index = 0; index < candidates.path("total").intValue(); index++) {
				...
				if (email != "" && email2 == "" && (email3 == "" || email3.equals("Not Applicable"))) {
					...
					query = "in:inbox (from:" + "(+" + email + ") OR from:" + "(+" + email2 + ")) after:"
							+ queryDateString;
					list = gmail
							.users()
							.messages()
					...
					query = "in:inbox (from:" + "(+" + email + ") OR from:" + "(+" + email3 + ")) after:"
							+ queryDateString;
					list = gmail
							.users()
							.messages()
					...
					query = "in:inbox (from:" + "(+" + email + ") OR from:" + "(+" + email2 + ") OR from:" + "(+"
				...
				if (list != null) {
					...
					for (final Message msgId : messages) {
						...
						toUpdate = entityApi.get(BHRestApi.Entity.ENTITY_TYPE.CANDIDATE, restToken,
								candidates.path("data").get(count).path("id").intValue(), "customDate1");

						updates = entityApi.update(BHRestApi.Entity.ENTITY_TYPE.CANDIDATE, restToken,
